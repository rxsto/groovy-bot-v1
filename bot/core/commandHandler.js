...
module.exports.run = async (Client, msg) => {
    ...
    if(!guild) await Client.functions.checkGuild(Client, msg.guild, Client.prefix, msg.guild.me.displayColor);
    ...
    if(msg.channel.type == "text" && msg.content.startsWith(guild.prefix) || msg.content.startsWith('<@'+Client.user.id+'>') || msg.content.startsWith('<@!'+Client.user.id+'>')) {
        ...
        if(cmd) {
            Client.log.log_info(`[Shard ${Client.shard.id + 1}] [CommandHandler] ` + colors.green(`${msg.author.username}#${msg.author.discriminator}`) + " @ " + colors.blue(`${msg.guild.name} (${msg.guild.id})`) + " in " + colors.cyan(`#${msg.channel.name}`) + " » ".yellow + msg.content);
            Client.log.write_info(`[Shard ${Client.shard.id + 1}] [CommandHandler] ` + (`${msg.author.username}#${msg.author.discriminator}`) + " @ " + (`${msg.guild.name} (${msg.guild.id})`) + " in " + (`#${msg.channel.name}`) + " » " + msg.content);
            if(guild.collector) {
