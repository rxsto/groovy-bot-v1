...
module.exports.run = (Client, msg, args, info) => {
    ...
    if(guild.loopSong) {
        ...
        if(info) {
            Client.functions.createEmbed(msg.channel, texts.loop_deactivated_text, texts.loop_deactivated_title);
        }
    } else {
        guild.loopSong = true;
        if(info) {
            Client.functions.createEmbed(msg.channel, texts.loop_activated_text, texts.loop_activated_title);            
        }
    }
}