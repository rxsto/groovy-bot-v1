...
module.exports.run = (Client, msg, args, info) => {
    ...
    if(guild.isShuffling) {
        ...
        if(info) {
            Client.functions.createEmbed(msg.channel, texts.shuffle_deactivated_text, texts.shuffle_deactivated_title);
        }
    } else {
        guild.isShuffling = true;
        if(info) {
            Client.functions.createEmbed(msg.channel, texts.shuffle_activated_text, texts.shuffle_activated_title);            
        }
    }
}