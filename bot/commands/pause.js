...
module.exports.run = async (Client, msg, args, info) => {
    ...
    if(guild.isPaused) {
        Client.functions.createEmbed(msg.channel, texts.command_pause_error, texts.error_title);
    } else {
        ...
        if(info) {
            Client.functions.createEmbed(msg.channel, texts.command_pause_text, texts.command_pause_text);
        }
    }
}