...
module.exports.run = (Client, msg, args, info) => {
    ...
    if(guild.loopQueue) {
        ...
        if(info) {
            Client.functions.createEmbed(msg.channel, texts.command_loopqueue_deactivated_text, texts.command_loopqueue_deactivated_title);
        }
    } else {
        guild.loopQueue = true;
        if(info) {
            Client.functions.createEmbed(msg.channel, texts.command_loopqueue_activated_text, texts.command_loopqueue_activated_title);            
        }
    }
}