...
module.exports.run = (Client, msg, args) => {
    ...
    if(!args[0]) {
        Client.functions.createEmbed(msg.channel, texts.general_no_arguments, texts.error_title);
    } else if(args[2]) {
        Client.functions.createEmbed(msg.channel, texts.general_to_many_arguments, texts.error_title);
    } else {
        ...
        if(first_song > len || second_song > len || first_song < 1 || second_song < 1) {
            Client.functions.createEmbed(msg.channel, texts.general_check_args, texts.error_title);
        } else {
            if(isNaN(first_song) || isNaN(second_song)) {
                ...
                if(fs == ss || fs == 0 || ss == 0) {
                    Client.functions.createEmbed(msg.channel, texts.command_move_cannot_switch, texts.error_title);
                    return;
                }
                ...
                Client.functions.createEmbed(msg.channel, texts.command_move_text, texts.command_move_title);
            }
        }
    }
}