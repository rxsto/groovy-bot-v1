...
module.exports.run = (Client, msg, args) => {
    ...
    if(args[1]) return Client.functions.createEmbed(msg.channel, texts.general_invalid_seek_position, texts.error_title);
    ...
    if(get == true) return Client.functions.createEmbed(msg.channel, texts.general_no_number, texts.error_title);

    if(position.length > 4 || position.length < 1) return Client.functions.createEmbed(msg.channel, texts.general_invalid_seek_position, texts.error_title);
    ...
    if(jumpto > guild.queue[0].info.length) return Client.functions.createEmbed(msg.channel, texts.general_number_to_big, texts.error_title);
    guild.process = jumpto / 1000;

    player.seek(jumpto);
    Client.functions.createEmbed(msg.channel, ":fast_forward: " + texts.command_forward_text + "`" + new Date(jumpto).toISOString().substr(11, 8) + "`!", texts.command_forward_title);
}