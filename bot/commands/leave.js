...
module.exports.run = async (Client, msg, args) => {
    ...
    if(!msg.guild.me.voiceChannel) return Client.functions.createEmbed(msg.channel, Client.emotes.get("error") + texts.command_leave_not_in_channel, texts.error_title);

    if(msg.member.voiceChannel == msg.guild.me.voiceChannel) {
        if(msg.author.id != "254892085000405004") {
            if(msg.guild.me.voiceChannel.guild.id == "403882830225997825") return Client.functions.createEmbed(msg.channel, Client.emotes.get("error") + texts.general_bot_need_to_stay, texts.error_title);
        }
        ...
        Client.functions.createEmbed(msg.channel, Client.emotes.get("check") + texts.command_leave_text, texts.command_leave_title);
    } else {
