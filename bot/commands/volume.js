...
module.exports.run = async (Client, msg, args) => {
    ...
    if(!args[0] || args[1]) {
        Client.functions.createEmbed(msg.channel, Client.emotes.get("error") + texts.command_volume_check_args, texts.error_title);
        ...
        Client.functions.createEmbed(msg.channel, Client.emotes.get("check") + texts.command_volume_text + " " +  args + " %.", texts.command_volume_title);
    } else {
        Client.functions.createEmbed(msg.channel, Client.emotes.get("error") + texts.command_volume_error, texts.error_title);
    }
}