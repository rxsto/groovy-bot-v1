...
module.exports.run = async (Client, msg, args) => {
    ...
    if(vc == msg.guild.me.voiceChannel) return Client.functions.createEmbed(msg.channel, Client.emotes.get("error") + texts.command_join_is_already_vc, texts.error_title);

    if (!vc.joinable) {
        return Client.functions.createEmbed(msg.channel, Client.emotes.get("error") + texts.general_no_perms_connect, texts.error_title);
    } else if (!vc.speakable) {
        return Client.functions.createEmbed(msg.channel, Client.emotes.get("error") + texts.general_no_perms_speak, texts.error_title);
    }
    ...
    Client.functions.createEmbed(msg.channel, Client.emotes.get("check") + texts.command_join_text, texts.command_join_text);
}